...
public class DodgingController : MonoBehaviour
{
    ...
    [SerializeField] private float hoverBobAmount = 0.1f;
    ...
    void ApplyHoverForce()
    {
        ...
        float verticalForce = hoverForce + (heightDiff * rb.mass * verticalSpeed);
        rb.AddForce(Vector3.up * verticalForce, ForceMode.Force);
        
        if (Mathf.Abs(heightDiff) < 0.5f)
        {
            rb.linearVelocity = new Vector3(rb.linearVelocity.x, rb.linearVelocity.y * 0.98f, rb.linearVelocity.z);
        }
    }
